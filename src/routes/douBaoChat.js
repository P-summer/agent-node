...
const { doubaoModel } = require("../llmModel/DoubaoLLM");
...
async function generateSessionTitle(userMessage) {
  ...
  try {
    // 调用模型生成标题（非流式，轻量调用）
    const response = await doubaoModel.invoke(titlePrompt);
...
const agent = createAgent({
  model: doubaoModel,
  tools: [serperSearchTool, searchMemory, saveMemory, timeTool],
